...
const source=new URL('../bootstrap/saved-reviews.json',import.meta.url);
const batch=()=>JSON.parse(readFileSync(source,'utf8'));
function harness(){const dir=mkdtempSync(join(tmpdir(),'saved-review-')),store=new Store(join(dir,'reviews.sqlite'));return {dir,store,close(){store.close();rmSync(dir,{recursive:true,force:true});}};}
...
test('imports three source-backed, unapproved reviews and preserves the existing review',()=>{
 const h=harness();try{
  ...
  const ids=importSavedReviews(h.store,source);assert.equal(ids.length,3);assert.equal(h.store.list().length,4);assert.deepEqual(h.store.get(previous.id),previous);
...
test('restarts never overwrite reviewer edits, history, or changed identities',()=>{
 const h=harness();try{
  const ids=importSavedReviews(h.store,source),j=h.store.get(ids[0]);j.plan.videos[0].sentences[0].text='A subsequent user edit';j.reviews=[{actor:'Keziah',decision:'request_changes',note:'Keep my feedback'}];j.identity='identity-after-a-rule-migration';h.store.save(j);
  const before=h.store.list();assert.deepEqual(importSavedReviews(h.store,source),[]);assert.deepEqual(h.store.list(),before);
 }finally{h.close();}
});

test('drafts missing Visual folders cannot reach HeyGen preparation even after script approval',()=>{
 const h=harness();try{
  const ids=importSavedReviews(h.store,source),jobs=ids.map(id=>h.store.get(id));
