...
test('local pilot inspect, example, review, permissions, failures, and exports',async()=>{
  ...
  try{
    ...
    assert.equal((await request(`jobs/${job.id}/review`,{index:0,decision:'approve',note:'Source reviewed carefully.',checkedEvidence:true},'Macy')).status,403);
    assert.equal((await request(`jobs/${job.id}/review`,{index:0,decision:'approve',note:'Source reviewed carefully.'},'Keziah')).status,400);
    job=(await request(`jobs/${job.id}/review`,{index:0,decision:'approve',note:'Automated QA exercise only. A team member must do the actual source review.',checkedEvidence:true},'Keziah')).body;
    ...
    assert.equal((await request(`jobs/${job.id}/videos`,settings,'Keziah')).status,403);
    ...
    assert.deepEqual(dispatches,[{id:job.id,actor:'Keziah',index:0}]);
