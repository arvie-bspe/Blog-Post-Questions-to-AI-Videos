...
test('SVG homepage logo is rasterised with Chromium and cached by domain',async t=>{
  ...
  try{
    const logo=new LogoService(dir,{analyzer:async()=>({faces:[],texts:[],relativeLuminance:.02}),fetcher:async url=>{calls.push(url);return url.endsWith('/brand.svg')?{url,headers:{'content-type':'image/svg+xml'},body:Buffer.from('<svg xmlns="http://www.w3.org/2000/svg" width="400" height="100"><rect width="400" height="100" fill="#253963"/><text x="20" y="65" font-size="40" fill="white">TEST FIRM</text></svg>')}:{url,headers:{'content-type':'text/html'},body:Buffer.from('<img alt="Firm logo" src="/brand.svg">')};}});
    const parent={client:{name:'Test Firm',homepage:'https://firm.example/'},row:{}};const first=await logo.acquire(parent),second=await logo.acquire(parent);assert.equal(first.domain,'firm.example');assert.equal(first.hash,second.hash);assert.equal(first.screening.relativeLuminance,.02);assert.equal(calls.length,2);assert.equal(readFileSync(first.path).subarray(1,4).toString(),'PNG');assert.equal(first.sourceContentType,'image/svg+xml');
  }finally{rmSync(dir,{recursive:true,force:true});}
});
