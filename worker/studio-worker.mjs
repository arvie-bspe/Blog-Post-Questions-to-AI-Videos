...
const types=[...(requestedTypes.has('ai_codex')?['ai_codex']:[]),...(requestedTypes.has('media_local')&&mediaPaths.every(value=>value&&existsSync(value))?['media_local']:[])];
...
async function media(task,dir){
  ...
  await run(process.env.LOCAL_MEDIA_PYTHON||'python',[join(root,'worker','local-media.py'),'render','--task',input,'--output',out],{cwd:root,timeout});
  ...
  return {engine:'kokoro-onnx+sadtalker',alignment:'kokoro-duration',completedAt:new Date().toISOString()};
}
