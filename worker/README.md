...
- `SSL_CERT_FILE` and `REQUESTS_CA_BUNDLE`: optional organization CA bundle paths when the worker's HTTPS connections require them.
...
Start the worker from this repository with `npm run worker`. Keep the computer awake and signed in. Stop it with Ctrl+C.
