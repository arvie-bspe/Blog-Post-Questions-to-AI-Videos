...
export class ScriptWorkflow{
  ...
  async startDirect(id,feedback='',index){
    ...
    if(scoped){
      ...
      job.questionRequests={...(job.questionRequests||{}),[index]:{index,requestId,draftHash:state.draftHash,feedback,status:'working',provider,attempt,at:new Date().toISOString()}};this.store.save(job);
      const submitted=this.directResolver.submit(job,{previousPlan,feedback,onlyQuestion:job.plan.videos[index].question,index,draftHash:state.draftHash,maxVideos:1,attemptToken:attempt,requestId});
      if(submitted.task){job.questionRequests[index].taskId=submitted.task.id;this.store.save(job);}else this.finishDirectPromise(submitted.promise,{jobId:id,index,draftHash:state.draftHash,sourceHash:job.doc.sourceHash,requestId});
      return job;
    }
    feedback=[...new Set([...(job.reviews||[]).filter(r=>r.decision==='reject').map(r=>r.note),feedback].filter(Boolean))].join('\n\n');
    const previousPlan=job.plan?structuredClone(job.plan):null;
    if(job.plan)job.history=[...(job.history||[]),{at:job.updated,plan:job.plan,reviews:job.reviews,questionAudits:job.questionAudits,questionRequests:job.questionRequests,origin:job.origin,audit:job.audit,validation:job.validation,revision:job.revision}];
    const requestId=randomUUID();
    // A full analysis starts a new audit generation, even if it returns identical text.
    job.status='analyzing';job.reviews=[];job.questionReviews=[];job.questionAudits={};job.questionRequests={};job.plan=null;job.error=null;job.analysisAttempt=(job.analysisAttempt||0)+1;job.origin=provider==='codex_worker'?'Local Codex worker draft with source audit':'Claude API draft with source audit';job.auditRequired=true;job.revisionRequest={feedback,status:'working',provider,requestId,at:new Date().toISOString()};this.store.save(job);
    const submitted=this.directResolver.submit(job,{previousPlan,feedback,maxVideos:job.maxVideos||2,requestId});
    if(submitted.task){job.analysisTaskId=submitted.task.id;this.store.save(job);}else this.finishDirectPromise(submitted.promise,{jobId:id,sourceHash:job.doc.sourceHash,requestId});
    return job;
  }
  ...
  currentDirectRequest(job,{index,taskId,requestId,sourceHash}){
    if(!job||job.mode!==directMode||sourceHash&&job.doc?.sourceHash!==sourceHash)return false;
    const scoped=Number.isInteger(index),request=scoped?job.questionRequests?.[index]:job.revisionRequest;
    if(!request||!['working','queued'].includes(request.status))return false;
    if(!scoped&&!['analyzing','interrupted'].includes(job.status))return false;
    // Existing queued tasks predate requestId; their persisted task ID is still authoritative.
    if(taskId)return (scoped?request.taskId:job.analysisTaskId)===taskId&&(!requestId||request.requestId===requestId);
    return Boolean(requestId&&request.requestId===requestId);
  }
  async applyTask(task){
    if(task.type!=='ai_codex'||task.appliedAt)return;
    const context={...task.payload?.context,taskId:task.id},current=this.store.get(context.jobId);
    if(!this.currentDirectRequest(current,context)){this.directResolver.tasks.markApplied(task.id);return;}
  ...
  async applyDirectResult(result,context){
    const {jobId,index,draftHash}=context,current=this.store.get(jobId);
    if(!this.currentDirectRequest(current,context))return current;
  ...
  failDirect(error,context){const {jobId,index}=context,job=this.store.get(jobId);if(!this.currentDirectRequest(job,context))return;if(Number.isInteger(index)){job.questionRequests[index].status='failed';job.questionRequests[index].error=error.message;updateQuestionStatus(job);}else{job.status='failed';job.error=error.message;job.revisionRequest.status='failed';}this.store.save(job);}
  failTask(task){
  ...
  async startQuestion(id,feedback,index){
    const job=this.store.get(id),state=questionState(job,index),key=id+':'+index,requestId=randomUUID();
    ...
    this.analyzer(job.doc,{...job.client,maxVideos:1},{previousPlan,feedback,onlyQuestion:job.plan.videos[index].question}).then(result=>{
      const current=this.store.get(id);if(current?.questionRequests?.[index]?.requestId!==requestId)return;if(questionHash(current,index)!==state.draftHash)throw new Error('This question changed while the rewrite was running. Review before retrying.');
    ...
    }).catch(e=>{const current=this.store.get(id);if(current?.questionRequests?.[index]?.requestId===requestId){current.questionRequests[index].status='failed';current.questionRequests[index].error=e.message;updateQuestionStatus(current);this.store.save(current);}}).finally(()=>this.active.delete(key));return job;
  }
