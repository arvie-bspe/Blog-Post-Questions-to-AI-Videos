...
export function reconcileRules(store){
  for(const job of store.list()){
    ...
    // Appearance-only changes do not manufacture, revoke or replay script reviews.
    if(visualOnly)job.scriptRulesHash=job.scriptRulesHash||job.rulesHash;else delete job.scriptRulesHash;
    ...
    job.rulesHash=rulesHash;job.rulesVersion=config.version;if(!visualOnly)job.reviews=[];if(perQuestion){job.questionReviews=[];job.questionRequests={};job.questionAudits={};}
    job.doc=withClientSelection(job.doc,job.client.key);
    if(!visualOnly)job.layoutNotice='Updated September 16: new script approval required. Previous scripts, feedback and videos remain in history.';
    ...
    if(!visualOnly&&(job.auditRequired===true||/^(?:OpenAI API|Local Codex worker|Claude API) draft\b/i.test(job.origin||'')))job.audit={passed:false,issues:['The content rules changed. Run a fresh source audit before approving this draft.']};
    ...
    try{
      ...
      store.save(job);store.record(job.id,'system',visualOnly?'appearance_rules_updated_content_approval_preserved':'global_rules_updated_review_required');store.db.exec('COMMIT');
